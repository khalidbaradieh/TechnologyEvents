...
// Shared data (news/settings) always comes from Firebase — no bust needed.
// Preserved: theme, lang, bookmarks (user preferences, not content).
(function _bustUserCacheIfNeeded() {
  try {
    const prev = localStorage.getItem(STORE.VERSION);
    if (prev !== VERSION) {
      // Wipe only content-derived local caches, not pure user prefs
      ['atq_view_counts', 'atq_viewed_articles',
       'atq_breaking_active', 'atq_breaking_start',
       'atq_cache_news', 'atq_cache_latest',
       'atq_cache_breaking', 'atq_cache_site'].forEach(k => {
...
// ── renderSite debounce ───────────────────────────────────────────
// Multiple onSnapshot listeners fire within milliseconds of each other on
// page load. Without debouncing, renderSite() runs 3+ times unnecessarily.
// This collapses rapid calls into a single render at the end of each burst.
let _renderTimer = null;
function _scheduleRender() {
  clearTimeout(_renderTimer);
  _renderTimer = setTimeout(renderSite, 16); // one animation frame
}

// ── Read-through cache: warm _fb from localStorage on page load ──
// On repeat visits this lets renderSite() fire instantly (0ms) before
// Firebase responds. Firebase then overwrites with fresh data (~300-800ms).
// On first-ever visit localStorage is empty so skeletons show as normal.
(function _warmCacheFromStorage() {
  try {
    const cn = localStorage.getItem('atq_cache_news');
    const cl = localStorage.getItem('atq_cache_latest');
    const cs = localStorage.getItem('atq_cache_site');
    const cb = localStorage.getItem('atq_cache_breaking');
    if (cn) { const p = JSON.parse(cn); if (Array.isArray(p) && p.length) _fb.news = p; }
    if (cl) { const p = JSON.parse(cl); if (Array.isArray(p))             _fb.latest = p; }
    if (cs) { const p = JSON.parse(cs); if (p && typeof p === 'object')   _fb.site = p; }
    if (cb) { const p = JSON.parse(cb); if (Array.isArray(p))             _fb.breaking = p; }
  } catch(_) {}
})();
...
// ═══════════════════════════════════════════════════════════════
// STATIC DEFAULT NEWS
// Empty — all content comes from Firebase onSnapshot.
// Array kept for backward compat with deep-link hash code only.
// ═══════════════════════════════════════════════════════════════
const STATIC_NEWS = [];

function getNewsData() {
  // Return empty until Firebase loads — skeletons show instead.
  // STATIC_NEWS is kept only for deep-link backward compat.
...
function checkBreaking() {
  ...
  // Also inject published news flagged with alsoBreaking or priority عاجل
  ...
  if (!allActive.length) { bar.classList.remove('visible'); return; }
...
function renderSite() {
  ...
  if (all.length > 0) {
    // Hero: swap loading→loaded to fade in real content
    const heroEl = document.querySelector('.hero-main.loading');
    if (heroEl) {
      heroEl.classList.remove('loading');
      heroEl.classList.add('loaded');
    }
    // News grid: remove skeleton cards, real cards injected below
    const mainGrid = document.getElementById('main-news-grid');
    if (mainGrid) {
      mainGrid.querySelectorAll('.news-card[style*="pointer-events:none"]').forEach(c => c.remove());
    }
    // Wide card: unhide when Firebase gives us real data
    const wideEl = document.getElementById('wide-card-el');
    if (wideEl && wideEl.style.display === 'none') wideEl.style.display = '';
    // Sidebar: remove skeleton cards (renderSite replaces them below)
  ...
  if (heroEl) {
    ...
    // Remove skeleton shimmer and transition to loaded state
    if (heroEl.classList.contains('loading') || heroEl.classList.contains('loaded')) {
      heroEl.classList.remove('loading');
      heroEl.classList.add('loaded');
      // Remove the skeleton span inside hero-img
...
function _applyNavMenu(items) {
  ...
  // Always re-append the mobile-only Contact Us li so hamburger menu includes it
...
function openArticleFromWide(el) {
  ...
  // Prefer Firebase site data; fall back to localStorage only if Firebase not yet loaded
...
(function init() {
  ...
  // ── Instant render from cache (repeat visits) ──────────────
  // If _warmCacheFromStorage() populated _fb.news, render immediately
  // so the visitor sees real content at 0ms instead of skeletons.
  // Firebase will still fire and silently refresh with the latest data.
  if (_fb.news && _fb.news.length > 0) {
    // Pre-flip hero to loaded so it never flashes as skeleton
    const heroEl = document.querySelector('.hero-main');
    if (heroEl) { heroEl.classList.remove('loading'); heroEl.classList.add('loaded'); }
    renderSite();
    // Also apply cached site settings immediately (title, buttons, ticker etc.)
    if (_fb.site) {
      const s = _fb.site;
    ...
    // Apply cached breaking state
    if (_fb.breaking) checkBreaking();
    // Open pending article from cache if possible
  ...
  // Start ticker RAF (fills from cache or waits for Firebase)
  _startTickerRAF();
  // Start Firebase real-time listeners — always run to get fresh data
  startFirebaseListeners();

  // Safety timeout: if Firebase takes unusually long (bad network / offline),
  // render whatever is available after 3 seconds so the page isn't blank forever.
  setTimeout(function() {
    if (!_fb.news) {
      console.info('[Site] Firebase timeout — rendering with empty data');
      renderSite();
    }
  }, 3000);
...
function startFirebaseListeners() {
  ...
  listenToCollection(DB.NEWS, (documents) => {
    ...
    // Write to localStorage cache — next visit renders instantly
    if (items.length) {
      try { localStorage.setItem('atq_cache_news', JSON.stringify(items.slice(0, 50))); } catch(_) {}
    }
    _scheduleRender();
  ...
  listenToDocument(DB.SETTINGS, DB.S.SITE, (data) => {
    ...
    try {
      ...
      // Don't cache large base64 logo images (can exceed 5MB localStorage limit)
    ...
    // Wide pinned changed → re-render (debounced)
    _scheduleRender();
